(function(window, factory) {
	if (typeof define === 'function' && define.amd) {
			...
			window.lightbox = factory(window);
	}
	...
	var _dataAriaHidden = 'lightbox-' + _attrAriaHidden;
	...
	var _defaultOptions = {
			...
			template: `
				<section class="lightbox" role="dialog" aria-label="Dialog Window (Press escape to close)" tabindex="-1">
					<div class="lightbox-wrap" data-lightbox-close role="document">
						<div class="lightbox-loader" aria-hidden="true">
							Loading...
						</div>
						<div class="lightbox-container">
							<div class="lightbox-content"></div>
							<button class="lightbox-close" type="button" aria-label="Close (Press escape to close)" data-lightbox-close>&times;</button>
						</div>
					</div>
				</section>
			`
	...
	function iframe(iframeUrl, instance, queryParams, hashUrl) {
			instance && instance.element().classList.add('lightbox-iframe');
			...
			return '<div class="lightbox-iframe-container"><iframe allow="autoplay; fullscreen" src="' + iframeUrl + '"></iframe></div>';
	}

	function error(msg) {
			var el = document.createElement('span');
			el.classList.add('lightbox-error');

			return el.appendChild(msg);
	}

	function imageHandler(target, instance) {
			var desc = (instance.opener() && instance.opener().dataset.lightboxDesc) || 'Image with no description';
	...
	function inlineHandler(target, instance) {
			...
			hasHideClass = el.classList.contains('lightbox-hide');

			instance
					.element()
					.addEventListener('lightbox:remove', function() {
							placeholder.insertAdjacentElement('beforebegin', el);
							if (placeholder.parentNode !== null) {
									placeholder.parentNode.removeChild(placeholder);
							}
							
							if (hasHideClass && !el.closest('.lightbox-content')) {
									el.classList.add('lightbox-hide');
							}
					}, { once: true })
			;
			
			el.classList.remove('lightbox-hide');
	...
	function registerInstance(instanceToRegister) {
			if (1 === _instances.unshift(instanceToRegister)) {
					_html.classList.add('lightbox-active');
	...
	function removeInstance(instanceToRemove) {
			...
			if (1 === _instances.length) {
					_html.classList.remove('lightbox-active');
			...
			if (!!_instances.length) {
					...
					show = document.querySelectorAll('.lightbox-hidden');
			}
	...
	function Lightbox(target, options, opener, activeElement) {
			var self = this;
			var result;
			var isReady = false;
			var isClosed = false;
			...
			self.resize = function() {
					...
					trigger(content, 'lightbox:resize', [self]);
			};
			...
			self.close = function() {
					...
					trigger(content, 'lightbox:close', [self]);

					element.classList.remove('lightbox-opened');
					element.classList.add('lightbox-closed');

					transitionEnd([content].concat(element))
							.then(function() {
									trigger(content, 'lightbox:remove', [self]);
									element.remove();
									element = undefined;
									deferred.resolve();
							})
			...
			element.classList.add('lightbox-loading', 'lightbox-opened', 'lightbox-' + result.handler);
			...
			trigger(element, 'lightbox:open', [self]);
			...
			function ready(result) {
					...
					$each(element.querySelectorAll('.lightbox-loader'), function() {
							var loader = this;
					...
					element.classList.remove('lightbox-loading');

					var el = element.querySelector('.lightbox-content');
					...
					trigger(content, 'lightbox:ready', [self]);
			}
	}

	function lightbox(target, options, opener) {
			if (!target.preventDefault) {
					opener = document.querySelector(opener);
			} else {
					target.preventDefault();
					opener = this;
					target = opener.dataset.lightboxTarget || opener.getAttribute('href') || opener.getAttribute('src');
			}
			...
			var instance = new Lightbox(
					...
					extend(
							{},
							opener ? opener.dataset.lightboxOptions || opener.dataset.lightbox : null,
							options
					),
					opener,
	...
	lightbox.version  = '4.0.0-dev';
	lightbox.options  = settings.bind(lightbox, _defaultOptions);
	lightbox.handlers = settings.bind(lightbox, _defaultOptions.handlers);

	lightbox.current  = currentInstance;
