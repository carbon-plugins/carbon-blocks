const SSR_MESSAGE = 'Animations were not initialised! Probably it is used in SSR.';
...
let options = {
  ...
  rootMargin: '0% 50%',
  threshold: 0.5,
  animateClassName: 'anim',
  disabledClassName: 'anim-disabled',
  enterEventName: 'anim:in',
  exitEventName: 'anim:out',
  selector: '[data-anim]',
  once: true,
  disabled: false,
};
...
const onIntersection = (entries, observer) => {
  entries.forEach((entry) => {
    const { target } = entry;
    const hasRepeatFlag = target.dataset.animRepeat !== undefined;
    const hasOnceFlag = target.dataset.animOnce !== undefined;
